...
- 主界面分为运行、声学、空间、设置四个底部页签；声学页再切换实时四图和探针详情，减少单页滚动长度
- 运行页和空间页都显示响应位置、朝向、水平仪及相机预览，并共享同一个持续 ARSession，切页不会重置世界坐标或中断空间扫描
- UDP 双向检验位于设置页；探针图和实时四图明确标注 FS、RIR 相对幅度、dBFS、kHz 纵轴单位及关键刻度
