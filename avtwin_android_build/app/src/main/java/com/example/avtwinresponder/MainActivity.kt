...
class MainActivity : Activity() {
    ...
    private var idleUdpTestListener: IdleUdpTestListener? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateFolderInfo()
        showIdleMetrics()
        restartIdleUdpTestListener()
    }

    override fun onResume() {
        super.onResume()
        if (::networkInfo.isInitialized) updateNetworkInfo()
        if (responder?.isRunning() != true) restartIdleUdpTestListener()
    }
    ...
    private fun buildUi() {
        ...
        root.addView(TextView(this).apply {
            text = "AV-Twin Continuous Acoustic Responder v0.9.2 ACK+POSE"
            textSize = 21f
        ...
        controlPort.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) { restartIdleUdpTestListener() }
        })
    ...
    private fun restartIdleUdpTestListener() {
        idleUdpTestListener?.stop()
        idleUdpTestListener = null
        if (!::controlPort.isInitialized || !::host.isInitialized || responder?.isRunning() == true) return
        val port = controlPort.text.toString().toIntOrNull() ?: return
        if (port !in 1..65535 || host.text.toString().trim().isBlank()) return
        idleUdpTestListener = IdleUdpTestListener(
            port = port,
            expectedLinuxHost = host.text.toString().trim(),
            onStatus = { message -> runOnUiThread { appendLog(message) } }
        ).also { it.start() }
    }
