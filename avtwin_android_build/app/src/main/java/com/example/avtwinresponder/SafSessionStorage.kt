...
class SafSessionStorage(
    ...
    private val manualPoseCache = StringBuilder()
    ...
    fun start(initialSessionJson: String): Handles {
        ...
        val manualPoseRecords = createFile(sessionDir, "text/csv", "manual_pose_records.csv")
        val logs = createFile(sessionDir, "text/plain", "logs.txt")
        val sessionJson = createFile(sessionDir, "application/json", "session.json")
        handles = Handles(sessionDir, events, manualPoseRecords, logs, sessionJson, audioDir, probesDir)

        writeText(sessionJson, initialSessionJson)
        val poseHeader =
            "session_id,measurement_id,t2_sample,pose_revision,x_m,y_m,z_m," +
                "yaw_deg,pitch_deg,roll_deg,qx,qy,qz,qw\n"
        manualPoseCache.append(poseHeader)
        writeText(manualPoseRecords, poseHeader)
    ...
    @Synchronized
    fun appendManualPose(measurementId: Long, t2Sample: Long, pose: ManualPose) {
        val h = handles ?: return
        ...
        appendText(h.manualPoseRecords, manualPoseCache, row)
    }
